...
class LessonReviewsFragment : Fragment() {
    ...
    ): View? = inflater.inflate(R.layout.fragment_lesson_reviews, container, false)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        llContainer = view.findViewById(R.id.llReviewsContainer)
        studentId   = requireActivity().intent.getStringExtra("USER_ID") ?: ""
        fetchCompletedLessons()
    }

    private fun fetchCompletedLessons() {
        if (studentId.isEmpty()) { showEmpty("Not logged in. Please restart the app."); return }
